...
pie.plot.data <- studentData.clean.class %>%
  filter((AVERAGEMODULEMARK >= 40) & (DEGREETYPE == 'BSc')) %>%
...
pie.plot <-
  ...
  ggtitle("Average Classifications of BSc students per Degree Type") +
  theme_void() +
...
#save plot as png
save_plot(pie.plot, 1500, 940, "BSc_avg_class_per_level_pie_chart.png")
