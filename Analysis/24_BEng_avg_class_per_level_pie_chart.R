...
pie.plot.data <- studentData.clean.class %>%
  filter((AVERAGEMODULEMARK >= 40) & (DEGREETYPE == 'BEng')) %>%
...
pie.plot <-
  ...
  ggtitle("Average Classifications of BEng students per Degree Type") +
  theme_void() +
...
#save plot as png
save_plot(pie.plot, 1500, 940, "BEng_avg_class_per_level_pie_chart.png")
